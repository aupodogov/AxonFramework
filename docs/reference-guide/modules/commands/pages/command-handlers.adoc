...
Command handlers can be **stateless** (not requiring any prior state) or **stateful** (depending on prior state to enforce business rules).
The different patterns for implementing stateful command handlers are covered in xref:entities/event-sourced-entity.adoc[Event-sourced entities] and xref:entities/command-handler-components.adoc[Command handler components].
...
When command handling requires entity state, you can use stateful command handlers as explained in more detail in xref:entities/event-sourced-entity.adoc[Event-Sourced Entities] and xref:entities/command-handler-components.adoc[Command Handler Components].
