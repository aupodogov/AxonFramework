...
:navtitle: Command handler components

A command handler component keeps command handling logic in a dedicated class, separate from the entity that holds state.
...
* The **command handler component** contains decision logic, validates the command against entity state, and appends events.

== Defining a command handler component
...
The command handler component is a plain class.
...
Axon resolves the entity ID from the `@TargetEntityId`-annotated field on `RenameCourse`.
...
Separate command handler components are the natural fit for https://www.jimmybogard.com/vertical-slice-architecture/[Vertical Slice Architecture] in Axon Framework.
...
* xref:commands:entities/event-sourced-entity.adoc[Event-sourced entities]—how to define entity state, event sourcing handlers, and configuration
* xref:commands:command-handlers.adoc[Command handlers]—command handlers and handler parameters
