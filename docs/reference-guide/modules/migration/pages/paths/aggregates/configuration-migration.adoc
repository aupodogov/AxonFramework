...
- xref:commands:entities/event-sourced-entity.adoc[Event-Sourced Entities]: reference guide for `@EventSourced`, `EventSourcedEntityModule`, and snapshot configuration in Axon Framework 5
- xref:commands:entities/index.adoc[Entities]: overview of all entity patterns and when to use each

[IMPORTANT]
.`@EventSourced` is not a full feature replacement for `@Aggregate`
The `@EventSourced` annotation in Axon Framework 5/Spring Boot is the configuration replacement for `@Aggregate`, but not a fully functional feature replacement. Attributes you may have used in AF4 (like caching, snapshotting) are no longer supported via annotation. xref:migration:paths/index.adoc#missing-a-migration-path[Contact us] if you require these use cases.
