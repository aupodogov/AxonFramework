...
+
New entities are created when a command appends an event that can be routed to an `@EntityCreator`. So any `@CommandHandler` that does not append such an event will implicitly follow the `NEVER` policy.

[#see-also]
== See also

For more information on command handling and modeling patterns in Axon Framework 5, refer to:

- xref:commands:entities/index.adoc[Entities]: stateful command handlers that enforce business rules using prior state
- xref:commands:entities/command-handler-components.adoc[Command handler components]: how to structure handlers by use case using Vertical Slice Architecture
...
- xref:commands:command-handlers.adoc[Command handlers]: stateless command handlers that need no prior state
