...
- Created `entities/event-sourced-entity.adoc` — full guide: `@EventSourcedEntity`/`@EventSourced`, `tagKey` (event stream filtering), routing via `@Command(routingKey)` / `@TargetEntityId`, separation of concerns, `EventAppender` metadata, snapshots, AF4 lifecycle migration notes (`markDeleted`, `isLive`, `@AggregateVersion` all removed)
- Created `entities/entity-hierarchies.adoc` — `@EntityMember` (replaces `@AggregateMember`), routing key, `RoutingKeyEventTargetMatcherDefinition`, single vs collection child entities, command routing resolution
  - Subsequent accuracy pass (this session): added Spring Boot / Plain Java tabs for parent entity definition and configuration; corrected event routing default (already `RoutingKeyEventTargetMatcherDefinition`—no "deliver to all" default exists); corrected exception to `ChildEntityNotFoundException` (not `IllegalStateException`); corrected single-child forwarding; added verified tests in `university-java-springboot-3` (`hierarchy/` package)
- Created `entities/entity-polymorphism.adoc` — `@EventSourcedEntity(concreteTypes = {...})`, `@EntityCreator` static factory method, constraints
  - Subsequent accuracy pass (this session): removed incorrect `@EventSourced` on subtypes guidance (real reason to avoid: unnecessary Spring prototype beans, not double-registration); added static `@CommandHandler` + `@EntityCreator` creation pattern table; fixed `event.typeData()` → proper typed fields (`event.platformUrl()` / `event.location()`) in subtype constructors; added Configuration section with Spring Boot / Plain Java tabs; corrected Constraints: unlisted subtypes silently ignored (no startup error), `DuplicateCommandHandlerSubscriptionException` for within-type duplicates vs `IllegalArgumentException` for creational clashes between subtypes, framework bypasses Java visibility via `setAccessible(true)` so `protected` is best practice not a hard requirement; verified all constraints against source code; added verified tests in `university-java-springboot-3` (`polymorphism/` package)
